...
export default function JsSummarize(options) {
  ...
  this._idealSentenceLength =
    options.idealSentenceLength || this._idealSentenceLength;
  this._returnCount = options.returnCount || this._returnCount;
  this._excludeList = options.excludeList || this._excludeList;
  this._positionValueArray =
    options.positionValueArray || this._positionValueArray;
}
...
JsSummarize.prototype.lengthScore = function (sentence) {
  return (
    1 -
    Math.abs(this._idealSentenceLength - sentence.length) /
      this._idealSentenceLength
  );
};
/**
...
JsSummarize.prototype.titleScore = function (title, sentence) {
  ...
  for (var i = 0; i < sentence.length; i++) {
    var word = sentence[i];
    if (!_.includes(this._excludeList, word) && _.includes(titleWords, word)) {
      count++;
    }
  }
...
JsSummarize.prototype.sentencePosition = function (index, numberOfSentences) {
  ...
  for (var i = 0; i < this._positionValueArray.length; i++) {
    ...
    if (normalized > position.low && normalized <= position.high)
